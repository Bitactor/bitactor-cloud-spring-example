package com.bitactor.cloud.spring.example.cluster.service2.frame;
...
@Slf4j
@Component
public class ProviderSenderPolicy implements ProviderChannelNettySendPolicy {
    ...
    public ChannelFuture sendHandler(MessageWrapper message, ChannelHandlerContext ctx, ChannelPromise channelPromise) {
        ...
        if (ctx.channel().isWritable()) {
            ...
            log.info("Do ProviderSenderPolicy method sendHandler  channelId:{},data type: {}", ctx.channel().id(), message.getClass().getSimpleName());
